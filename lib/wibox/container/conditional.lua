...
-- met.
--
-- If multiple conditions are met, the first one is used.
--
...
--
-- * **when** (*function*): The function called to determine if
...
--  selected.
-- * **updated_callback** (*function*): When `eval` is executed, but the
--  selected condition remains the same has the previons evalutation.
--
...
function conditional:set_children(_)
...
function conditional:set_templates(templates)
    for k, v in pairs(templates) do
        ...
        -- Without a way to determine the selected template, this will explode.
...
function conditional:eval()
    ...
    if not self._private.templates then return end

    local callback_args = {self._private.argument_callback(self)}

    for _, v in ipairs(self._private.templates) do
        if v.when then
            if v.when(unpack(callback_args)) then
                selected = v
                break
            end
        elseif self._private.default_when then
            local ret = self._private.default_when(v, unpack(callback_args))
...
--
-- @method conditional:attach_signal
...
-- @method detach_signal
-- @tparam hears.object object Any AwesomeWM API object.
-- @tparam string signal The signal name.
...
-- @constructorfct wibox.container.conditional
