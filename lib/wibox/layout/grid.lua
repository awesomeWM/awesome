...
--- A grid layout.
--
-- Widgets spanning several columns or rows cannot be included using the declarative system.
...
-- If false, check before adding a new widget if it will superpose with another widget and prevent from adding it.
--
...
-- Get the value `horizontal_homogeneous` or `vertical_homogeneous` defined by the preferred `orientation`.
--
--@DOC_wibox_layout_grid_expand_EXAMPLE@
...
local function find_widgets_at(widgets_table, row, col, row_span, col_span)
    ...
    for index, data in ipairs(widgets_table) do
        ...
        local test_horizontal = not (row > data.row + data.row_span - 1 or data.row > row + row_span - 1)
        -- If one rectangular widget is above other
        local test_vertical   = not (col > data.col + data.col_span - 1 or data.col > col + col_span - 1)
...
function grid:get_next_empty(hint_row, hint_column)
    ...
    while true do
        if find_widgets_at(self._private.widgets, row, column, 1, 1) == nil then return row, column end
...
function grid:add_widget_at(child, row, col, row_span, col_span)
    ...
    local superpose = find_widgets_at(self._private.widgets, row, col, row_span, col_span)
...
-- @treturn table The `position` table of the coordinates in the grid, with fields `row`, `col`, `row_span` and `col_span`.
...
function grid:insert_column(index)
    if index == nil or index > self._private.num_cols + 1 or index < 1 then index = self._private.num_cols + 1 end
...
function grid:extend_column(index)
    if index == nil or index > self._private.num_cols or index < 1 then index = self._private.num_cols end
    -- Update widget positions
...
function grid:insert_row(index)
    if index == nil or index > self._private.num_rows + 1 or index < 1 then index = self._private.num_rows + 1 end
    -- Update widget positions
...
function grid:extend_row(index)
    if index == nil or index > self._private.num_rows or index < 1 then index = self._private.num_rows end
    -- Update widget positions
...
-- getting the common property returns the directional property defined by the `orientation` property
...
function grid:reset()
    ...
    self._private.num_rows = self._private.forced_num_rows ~= nil and self._private.forced_num_rows or 0
    self._private.num_cols = self._private.forced_num_cols ~= nil and self._private.forced_num_cols or 0
    -- emit signals
    self:emit_signal("widget::layout_changed")
...
local function new(orientation)
    ...
    local dir = (orientation == "horizontal" or orientation == "vertical") and orientation or "vertical"
